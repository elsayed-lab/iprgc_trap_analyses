...
old_options <- options(stringsAsFactors = FALSE, knitr.duplicate.label = "allow")
...
Note to self: I changed the clusterProfiler output list to store the
enrichResults in [[xx_data]][[enrich]], or in the case of go:
[[go_data]][[BP_enrich]] instead of enrich_results.

# Set some parameters which will be used later

```{r}
sample_sheet <- "sample_sheets/20240606_only_umd_sequenced_modified.xlsx"
## LogFC cutoff when working on 'normal' comparisons
lfc_cutoff <- 1.0
## LogFC cutoff when working on the inclusion sets.
filter_lfc_cutoff <- 0.1
## Adjusted p-value cutoff when working with 'normal' contrasts
adjp_cutoff <- 0.05
...
annotation_columns <- c("ensembl_gene_id", "ensembl_transcript_id",
                        "description", "external_synonym")
label_column <- "symbol"  ## taken from org.Mm.eg.db
```
...
# A note on transcript-level changes

An interesting question was posed regarding transcript-level changes
in the TRAP and bulk data.  As a result, I made copies of the
notebooks and repeated all analyses at the transcript level; the
various images, excel files, etc are all in the same directories with
a suffix '_tx'.  A few minor changes were introduced for the labelling
of genes.

I made no effort to compare the similarities between the
transcript-level changes for the TRAP and bulk data; but did that only
...
any analyses depending on those gene sets will fail a priori.
...
I added a little code to my cyoa package which downloads all the
tables at https://ftp.ensembl.org/pub/current/mysql/ for either the
biomart database (ensembl_mart_116/ and/or mouse_mart_116) or the
species specific database (mus_musculus_core_116_39); I then added
some functions to hpgltools to directly query these local SQL
databases.  This is crazy fast (< 2 seconds vs. ~ 2 minutes) compared
to using biomart, but I do not entirely understand the schemas yet and
so miss some potentially useful columns of interest.  If anyone is
interested, I would be happy to pass along more complete instructions;
I am thinking it would be nice for the community to make the lives of
people at ensembl easier.

### My strategy for the container

I decided to load my existing ensembl annotations as the canonical
set, then add to it a portion of the mm.eg.db annotations.  I then
split them to one set for the gene data and another for the
transcripts.

Here are the most important label columns:

1.  symbol: The gene symbol (provided by org.Mm.eg.db)
2.  external_transcript_name: Usually the same as symbol, but with a
    suffix identifying which transcript it refers to. (from biomart)
3.  genename: Long-form name (from org.Mm.eg.db)
4.  description: Often but not always identical to the genename,
    includes accessions for a couple of other databases. (from
    biomart)
5.  ensembl_gene_id: The ENSMUSG id, our primary key for genes
6.  ensembl_transcript_id: The ENSMUST, our primary key for transcripts

Due to the way I am merging these, they are pretty redundant, but they
will get pared down when merged to the assay data.
...
The end result of these various annotation collections is mostly
identical, but the column containing the MGI gene names may get
changed.  This is pretty annoying when it comes to getting the
container image to run perfectly in all situations; and so at this
time it just loads the local.rda file; if one wishes to play press
your luck with ensembl, just delete the annotation rda and re-render
the documents.
...
```{r}
mm38_hisat_v2 <- create_se(sample_sheet, gene_info = mm_annot_genes,
                           file_column = "hisat_count_table") |>
  set_conditions(fact = "geno_loc_atb") |>
  set_batches(fact = "time_atb") |>
  set_colors(color_choices[["geno_loc"]])
mm38_hisat_v2
mm38_hisat_v3 <- create_se(sample_sheet, gene_info = mm_annot_genes,
...
# Use the Summarized Experiment to prune the gene annotations

```{r}
mm_annot_genes <- as.data.frame(rowData(mm38_hisat_v3))
```

## Archive count tables for the singularity container

The following is what I did to make an archive of the transcript count
tables following deduplication.  It creates a tar archive of all the
filenames provided in the 'column' argument.

```{r, eval=FALSE}
mrna_tables <- tar_meta_column(mm38_hisat_v3, column = "umi_dedup_mrna_count")
```
...
pp(file = "01diagnostic_images/nonzero_v2_filt.pdf", image = v2_nonzero_filt[["plot"]])

v3_nonzero_filt <- plot_nonzero(mm38_hisat_v3, plot_labels = FALSE)
pp(file = "01diagnostic_images/nonzero_v3_filt.pdf", image = v3_nonzero_filt[["plot"]])
```
...
v2_norm_pca
pp(file = "01diagnostic_images/v2_norm_pca.pdf", image = v2_norm_pca[["plot"]])

v3_norm <- normalize(mm38_hisat_v3, transform = "log2", convert = "cpm",
                     norm = "quant", filter = TRUE)
v3_norm_pca <- plot_pca(v3_norm)
pp(file = "01diagnostic_images/v3_norm_pca.pdf", image = v3_norm_pca[["plot"]])
```
...
cleanup, and library prep/sequencing. We know from Rashmi's QC
...
pp(file = "01diagnostic_images/location_genotype_tmm_pca.pdf",
   image = location_genotype_tmm_pca[["plot"]])
```
...
pp(file = "01diagnostic_images/design_sankey.pdf", image = sample_sankey[["plot"]])
```
...
pp(file = "images/ggbetween_location.pdf", image = opn4_location)
opn4_genotype <- ggstatsplot::ggbetweenstats(data = opn4_exprs, x = genotype, y = opn)
pp(file = "images/ggbetween_location.pdf", image = opn4_genotype)
opn4_combined <- ggstatsplot::ggbetweenstats(data = opn4_exprs, x = combined, y = opn)
pp(file = "images/ggbetween_combined.pdf", image = opn4_combined)
```
...
pp(file = "01diagnostic_images/v3_location_norm_pca.pdf", image = v3_location_pca)
```
...
pp(file = "02filtered_images/filtered_location_pca.pdf", image = filtered_location_pca)

removed_sankey <- plot_meta_sankey(v3_location, color_choices = color_choices,
                                   factors = c("genotype_atb", "location_atb", "time_atb"))
pp(file = "02filtered_images/filtered_sankey.pdf", image = removed_sankey[["plot"]])
```
...
'external_synonym' so that I need not change any of her code.  I am
...
annots_to_merge <- mm_annot_genes |>
  dplyr::select(ensembl_gene_id, ensembl_transcript_id, symbol) |>
...
My April 2025 version of this shows 21,793 and 3,632 genes in this
set.  Is that still true?  (As of 20260311, it is!)
...
## Important 202608 note

IMy problems with ensembl have resulted in me loading the transcript
IDs rather than the MGI symbols.  I hope to resolve this as soon as
possible.  For the moment I just want to make certain that all of the
data points match previous revisions of the analysis.

```{r}
head(df)

df <- df |>
  filter(rownames(df) %in% keepergenes)
labels_ups <- df |>
...
res_tbl <- df
DEplot <- ggplot(res_tbl, aes(x = logFC, y = -log10(adj.P.Val), label = symbol)) +
...
regulated_genes <- res_tbl |>
  ...
  arrange(logFC) |>
  dplyr::select(gene_id, logFC, adj.P.Val, symbol, Significance) |>
  dplyr::filter(abs(logFC) >= 1.0)
## gsea_result_ko <- gost(query = ko_genes$external_synonym,
##                        organism = "mmusculus",
##                        evcodes = TRUE,
##                        ordered_query = TRUE)
gsea_result_het <- gost(query = het_enriched$symbol,
                        organism = "mmusculus",
                        evcodes = TRUE,
                        ordered_query = TRUE)
##gsea_result_alldysregulated <- gost(query = alldysregulated_genes$external_synonym,
##                                    organism = "mmusculus",
##                                    evcodes = TRUE,
##                                    ordered_query = TRUE)
summary(gsea_result_het)
```
...
pp(file = "03theresa_comparison_images/gsea_p08_retina_het_vs_wt_top_hit.pdf",
   image = print(p08_topn_gsea[["GO_het_vs_wt_up"]][[1]]))
```
...
```{r}
dedup_query <- colData(mm38_hisat_v3)[, c("hisat_genome_single_all", "umi_dedup_pct_reads")]
test_plot <- plot_linear_scatter(dedup_query, loess = TRUE)
pp(file = "01diagnostic_images/deduplication_vs_hisat_scatter.pdf", image = test_plot[["scatter"]])
```
...
pp(file = "01diagnostic_images/sample_boxplot.pdf", image = box)
```
...
Note: The following few blocks are all copy/pasted directly from
Theresa's notebook and are not evaluated because they are performed
almost identically later but with slightly different logic/orders.
...
annots_to_merge <- mm_annot |>
  select(ensembl_gene_id, external_synonym) |>
...
labels <- rbind(labels_ups, labels_downs)
res_tbl <- df
DEplot <- ggplot(res_tbl, aes(x = logFC, y = -log10(adj.P.Val), label = external_synonym)) +
  geom_point(aes(colour = Significance), size = 4) +
  geom_vline(xintercept = c(-1, 1)) +
  geom_hline(yintercept = -log10(0.05)) +
  theme_classic(base_size = 20) +
  ...
  geom_label_repel(data=filter(df,
                               ...
                               external_synonym %in% labels$external_synonym),
                   ## nudge_x = -0.5,
                   nudge_y = 3, max.overlaps = 15) +
...
ko_genes <- res_tbl |>
  ...
  arrange(-abs(logFC)) |>
  select(Row.names, logFC, adj.P.Val, external_synonym, Significance) |>
  filter(logFC <= -1)
...
gsea_result_het <- gost(query = het_genes$external_synonym,
                        organism = "mmusculus",
                        evcodes = TRUE,
                        ordered_query = TRUE)
...
scn_enriched <- df |>
  ...
  arrange(-logFC) |>
  select(Row.names, external_synonym, logFC, adj.P.Val) |>
...
notenriched <- df |>
  select(Row.names, external_synonym, logFC, adj.P.Val, Significance) |>
...
DEplot <- ggplot(res_tbl, aes(x = logFC, y = -log10(adj.P.Val), label = external_synonym)) +
  ...
  geom_label_repel(data=filter(df,
                               external_synonym %in% labels_requested),
                   ## c(labels$external_synonym, "Opn4")), #c('s5_het_dlgn', 's5_het_ret', 's5_het_scn')),
                   ## nudge_x = -0.5,
...
```{r, eval=FALSE}
gsea_result_scn <- gost(query = scn_enriched$external_synonym,
                        organism = "mmusculus", evcodes = TRUE,
                        ordered_query = TRUE, source = c("GO"))

gsea_result_ret <- gost(query = retina_enriched$external_synonym,
                        organism = "mmusculus", evcodes = TRUE,
                        ordered_query = TRUE, source = c("GO"))
```
...
DEplot <- ggplot(res_tbl, aes(x = logFC, y = -log10(adj.P.Val), label = external_synonym)) +
  ...
  geom_label_repel(data = filter(
    df, external_synonym %in% c(labels$external_synonym, "Opn4")),
    ## c('s5_het_dlgn', 's5_het_ret', 's5_het_scn')),
    ## nudge_x = -0.5,
...
input_heatmap <- gplots::heatmap.2(as.matrix(input_mtrx), scale = "none", trace = "none",
...
### Potential text describing filtering

This dataset includes wild-type mice for each condition; this provided
an opportunity to filter out the most likely false positive
genes/transcripts.  When the samples were processed and the tagged
large ribosomal subunits were pulled down, it was assumed that at
least some other transcripts were likely to bind to the column and
therefore end up in the sequencing libraries.  If one assumes that
this distribution is consistent over time, genotype, and location;
then a likely strategy would be to take the mean value of all observed
wild-type samples and exclude any genes/transcripts for which the
heterozygous and/or knockout samples have lower observed abundance
than this mean.  We therefore performed the comparisons of the
wild-type samples by time and location and observed significant
variance; thus we chose instead to exclude genes/transcripts which are
lower in the heterzygous and knock-out samples than any of the comitant
wild-type conditions.  As an example, if the desired contrast is to
compare heterzygous retina / knockout retina at p08; then the set of
observed abundances was first compared against the p08 wt retina
samples and filtered to include only those which were deemed higher
when comparing the (p08 heterozygous retina / p08 wild-type retina)
and (p08 knockout retina / p08 wild-type retina) via DESeq2, with a
less stringent definition of significant (adjusted p-value <= 0.1 and
logFC >= 0.1).  Two versions of this filter were applied, one which
required either the numerator or denominator to be higher than the
corresponding wild-type samples; and one which required both to be
higher than the wild-type samples.  The mean normalized expression
values were reported with each of these comparisons so that one may
perform other filters as desired.

I will put this slightly differently, copy-pasting the observed set of
genes increased in the non-wt condition to show the result:
...
When comparing against the dLGN wild-types, there are between 180 and
2,544 genes which pass the filter.  The retina filter passes between
347 and 1,551 genes.  The SCN filter passes far fewer genes, ranging
from 8 to 225.

If we filter based on the intersection of genes which pass these, then
any contrast which includes SCN samples will have ~ 0 genes of
interest by definition.  Thus, we chose to allow genes which pass the
union of either the numerator or denominator filter.  In this instance,
we retain a significant number of genes as long as we are not
restricted to only SCN samples.
...
## 202608: Recapitulated against the 202603 results!
...
202608: I am seeing a tiny shift in p-values/logFC across all contrasts and
methods; it is so small that normally I would not see it.  In
addition, if one looks at the metadata tab in the output pairwise xlsx
file (inclusion_tables-v20250515.xlsx compared to
inclusion_tables-v20260820.xlsx for example), they include the mean
values and surrogate variables from sva -- these are exactly identical
to the number of digits available in excel/R.  We can go one step
further and observe that the various metrics of beasmean and such.

"""
1.  When we start the DESeq to generate the set of included genes:
  a.  The input data is exactly identical between 202503 and today --
  I can see that in a few places, when comparing the xlsx outputs the
  easiest are the % variance in the PCA plots on sheet #1 and the
  basemean values in all the following sheets.  They are identical for
  every gene on every sheet I checked (~10 genes on 3 sheets each
  ranging across the top and bottom 500 most expressed genes).
  b.  The first step of the DE analysis is to invoke sva and calculate
  the additions to the model.  These get printed to the metadata sheet
  of the xlsx file and are exactly identical.

  So, I know for certain that the input to all methods is the same
  from 202503 to now.  I also know for certain that every model got
  the same surrogates.

2.  The logFC values for every method have shifted by between 0 and
    0.01, including my basic method.
3.  The p-values have shifted by between 0 and 0.0001 for every method.
4.  The adjusted p-values have shifted by between 0 and 0.001 for every method.

I have gone through every line which performs the differential
expression and I do not see any changes between 2025 and now.

The end result is that the set of 'significant' genes for every
contrast in every comparison is exactly the same.  However, we are
extracting genes which are _insignificantly_ higher in the condition
vs. wt and this set of genes has decreased by between 0 and 10 genes
across the comparisons which in the case of the SCN samples which
already have very few results in a noticeable difference.

"""

Let us try a very slightly different filter when invoking DESeq2?
Damnit I don't know.  The following is the invocation produced by
all_pairwise()

```{r}
test_diff <- deseq2_pairwise(v3_pairwise_input, filter = "simple",
                             model_fstring = default_fstring, keepers = inclusions,
                             model_svs = "svaseq", null_fstring = "~", extra_contrasts = NULL,
                             annot_df = NULL, force = FALSE, deseq_method = "long",
                             fittype = "parametric", num_surrogates = "be", keep_underscore = TRUE)
```

I am running that under the debugger, I am now at the step which
produces the method-specific surrogate variables; when run under
all_pairwise, it produces the SVs for all methods at the same time and
passes them along.  SV3 is the most sensitive to differences, so I
will check the first few entries for SV2 and SV3.  Sample iprgc_86 in
the 202503 xlsx file has SV3 as -0.1657932.  In my R session I see
exactly that.  I went on to check SVs 1,2,3 for the first 18 samples,
they are all identical.

One weird little change I did notice, I have some logic to check if
the input is filtered; one term was backward and I fixed it.  I do not
think that matters; but one test I will apply is to repeat the DE
analysis with the pre and post filtered inputs.

ok, I completed the DESeq run under the debugger and recapitulated the
differences observed between 202503 and today.  The baseMean (column
AK in the xlsx file) values are identical, but the SE/stat/FC/p/adjp
are slightly different for most genes.

Let us now rerun with the pre-filtered set as input and leave the SVs
with their current values (which are identical to the 202503 run)?

Note to self, when I finish this: there is a hard-coded number of
significant digits when I print out the table of results (4), don't do
that -- set an option for that.

Ok, this unfiltered run results in a set of results which now has
25,425 rows, so it definitely did not filter.  In addition, the
adjusted p-values have shifted by ~ 1e-5, but the logFC and p-values are
identical to the 202608 run.

So that is not the answer but at least I was not wrong that it has an
effect, I guess.  Let us try the opposite filter -- switch to CBCB
filtering which was the default some time ago?  Filtering really is
the only thing I can think of which would change results across
_every_ method by such a small amount.  Same result.

ok, I checked the result; same.  I changed the betaprior, the deseq
normalization, and significant digits.  No change.

Random thought: I set the number of significant digits to 9 and tried
again.  Aaaannnndddd I hate everything - that is the answer.  I think
I need to make certain that options(digits = x) where x is consistent.
...
pp(file = "05inclusion_volcano_ma/hw_p08_dlgn_volcano.pdf", width = 9, height = 9,
   image = hw_p08_dlgn_volcano[["plot"]])
...
pp(file = "05inclusion_volcano_ma/kw_p15_dlgn_volcano.pdf", width = 9, height = 9,
   image = kw_p15_dlgn_volcano[["plot"]])
...
pp(file = "05inclusion_volcano_ma/kw_p08_dlgn_volcano.pdf", width = 9, height = 9,
   image = kw_p08_dlgn_volcano[["plot"]])
...
pp(file = "05inclusion_volcano_ma/hw_p15_retina_volcano.pdf", width = 9, height = 9,
   image = hw_p15_retina_volcano[["plot"]])
...
pp(file = "05inclusion_volcano_ma/hw_p08_retina_volcano.pdf", width = 9, height = 9,
   image = hw_p08_retina_volcano[["plot"]])
...
pp(file = "05inclusion_volcano_ma/kw_p15_retina_volcano.pdf", width = 9, height = 9,
   image = kw_p15_retina_volcano[["plot"]])
...
pp(file = "05inclusion_volcano_ma/kw_p08_scn_ma.pdf", width = 9, height = 9,
   image = kw_p08_scn_ma[["plot"]])
```

There is something different here compared to the 202503 results.  Two
of the dots have moved; but every other plot looks exactly the same.
...
extract_inclusions <- function(inclusion_sig, inclusion_tables, inclusions, keepers, all_genes,
  ...
  for (c_num in seq_along(keepers)) {
    ...
    written_inclusion <- write_xlsx(data = included_df,
                                    excel = glue("07included_genes/{include_name}-v{ver}.xlsx"))
    retlist[[include_name]] <- include_genes
    retlist[[contrast]] <- include_genes
  }
  return(retlist)
...
for (k in seq_along(genotype_keepers)) {
  ...
  if (num_rows >= 10) {
    ...
    #genotype_cp_test[[name]] <- all_cprofiler(
    #  genotype_sig[[name]], genotype_tables[[name]],
    #  orgdb = "org.Mm.eg.db", orgdb_from = orgdb_from, organism = "mouse")
